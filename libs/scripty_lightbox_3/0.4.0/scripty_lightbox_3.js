document.observe('dom:loaded', function(){
  ...
  $$('a.popup').each(function(elm){
    ...
    elm.observe('click',function(evt){
      ...
      if(geometry && (!extension || ! $w('mov m4v mp4 jpg jpeg png gif').include(extension))){
      ...
      if(geometry && $w('mov').include(extension)){
        player.setStyle('width:' + (geometry.width) + 'px; height:' + (geometry.height + 16) + 'px;');
      ...
      if(geometry && $w('m4v mp4').include(extension)){
        player.setStyle('width:' + (geometry.width + 30) + 'px; height:' + (geometry.height + 36) + 'px;');
        var qt = new Template('<p><video style="width: #{width}px; height: #{height}px;" src="#{src}" controls #{autoplay}></video></p>');
        vars = {'height':geometry.height, 'width':geometry.width, 'src':elm.href, 'autoplay': (autoplay == 'true' ? 'autoplay' : '')};
        player.update(qt.evaluate(vars));
        window.setTimeout(center_player,1);
      }
